...
public:
    ...
    std::vector<OutputVoxel> snapshot(
...
private:
    ...
    // Key is defined at namespace scope, not inside SemanticFusionMap.
    Key keyFor(
        double x,
        double y,
        double z) const;
    ...
    uint32_t classes_;
    double prior_;
    double decay_seconds_;

    mutable std::mutex mutex_;

    std::unordered_map<Key, VoxelState, KeyHash> map_;
};
